\paragraph{From binary to continuous.}
\citet{matheson1976scoring} show that CRPS inherits strict propriety from a simpler object: at each threshold $z$, the binary quadratic scoring rule evaluates how well $F_\text{pred}(z)$ predicts the indicator $\mathbf{1}\{y \le z\}$, and integration over all thresholds aggregates these binary assessments into a single continuous score. The indicator is deterministic---$0$ or $1$ depending on whether $y \le z$. Replacing it with a smooth $F_\text{obs}(z) \in [0, 1]$ changes each binary observation from certain to probabilistic but preserves the $L^2$ integration structure. This motivates a direct generalisation; its consistency properties are verified in \S5.

\paragraph{Reformulation.}
Instead of comparing the forecast against a hard step at the reported value, I compare it against a smooth distribution that represents what is actually known about the observation. Concretely, I replace the step indicator in the CRPS integrand with a smooth observational CDF $F_\text{obs}(z)$:
%
\begin{equation}
...
The integral in Eq.~\ref{eq:fuzzy_crps} is a known mathematical object---the Cram\'er distance between $F_\text{pred}$ and $F_\text{obs}$, measuring the total squared area between two probability curves \citep{cramer1946}.\footnote{The Cram\'er distance is closely related to the energy distance of \citet{szekely2013energy}: $\int(F - G)^2\,dz = \tfrac{1}{2}\,\varepsilon(F,G)$ in the univariate case.} In forecast verification, \citet{thorarinsdottir2013using} define the same integral as the \emph{Integrated Quadratic Distance} (IQD) and apply it to evaluate climate model output against reanalysis distributions---both non-degenerate CDFs. Classical CRPS is the special case where $F_\text{obs}$ degenerates to a step function at a single point; Eq.~\ref{eq:fuzzy_crps} is the general form applied to a non-degenerate $F_\text{obs}$. The same generalisation arises independently in the ``observational-probability'' approach to ensemble verification \citep{candille2008impact}, where point observations are replaced by probability distributions. \citet{pinson2012verification} implement this via Monte Carlo simulation---averaging classical CRPS over draws from $F_\text{obs}$---which differs from Eq.~\ref{eq:fuzzy_crps} by a constant that depends only on $F_\text{obs}$ and therefore preserves forecast rankings. (Some authors reserve ``Cram\'er distance'' for the square root of this integral, following the convention that a distance inherits the $L^2$ norm; I use it throughout for the integral itself, following \citealt{thorarinsdottir2013using}.)
